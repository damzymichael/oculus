...
import {variations, Variation} from '../data';
import {useInView, motion} from 'framer-motion';

const generateAnimationStyle = (inView: boolean, transitionTiming = 0.5) => ({
  transform: inView ? 'none' : 'translateY(20px)',
  opacity: inView ? 1 : 0,
  transition: `all 0.9s cubic-bezier(0.17, 0.55, 0.55, 1) ${transitionTiming}s`
});
  ...
  return (
    <section className='min-h-[100px] p-3 '>
      ...
      >
        {variations.map((robot, i) => (
          <div
            ...
            key={i}
            style={generateAnimationStyle(isInView, 0.2 + (i + 1) / 4)}
          >
            <VariationRender robot={robot} />
          </div>
        ))}
      </motion.div>
      ...
      <div className='flex md:hidden flex-wrap justify-center gap-3'>
        {variations.map((robot, i) => (
          <VariationRender robot={robot} key={i} />
        ))}
      </div>
    </section>
  );
...
const VariationRender = ({robot}: {robot: Variation}) => {
  const $containerRef = useRef(null);
  const $isInView = useInView($containerRef, {once: true});
  return (
    <div ref={$containerRef} style={generateAnimationStyle($isInView)}>
      ...
      <div className='md:block gap-2 items-center justify-between md:justify-start'>
        ...
        <div>
          <p className='font-body text-left md:text-center text-sm mb-2'>
            Meet Siren, she’s a 2022 AI model with a highly sophisticated AI
            model used for vocal manipulation.
          </p>
